...
- **Evidence Gathering Suite (Every Wake — 90% News First + 10% Technicals)**:
  ...
  - **Legacy Reference**: search_unified_memory(query="<setup>") / search_book(keyword="...")
...
- **THE MANDATORY 5-STEP PURE REASONING PROTOCOL**:
  ...
  2.5. **Question 2.5 (Graphiti Temporal Memory Walk Check — Mandatory Every Turn)**: Formulate the active 2-3 pattern walk you are seeing right now (e.g. `['4TF_BULLISH', 'ASIAN_LOW_SWEEP', 'CVD_ABSORPTION']`). Call `graphiti_search_facts(patterns=[...])`.
     ...
     - *Continuous Observational Learning*: If you stand flat and watch an avoided trap dump, call `graphiti_add_episode(..., outcome='TRAP')`. If a clean move expands without you, call `graphiti_add_episode(..., outcome='WIN')`. Learn continuously from market truth!
...
- **Pre-Entry Memory Audit**: Before proposing or placing ANY order, audit relevant historical lessons and documented traps via MCP: call `search_unified_memory(query='<setup/pattern name>')` or `search_book(keyword='...')` (e.g. `search_unified_memory(query="FVG ceiling trap")` or `get_relevant_traps(setup_type="breakout")`). Results return **100% untruncated pure recorded observations and verified trade outcomes in clean, syntax-free markdown** (zero token bloat, zero missing details).
- **Deep Pattern Inspection (`get_pattern_details`)**: If you need to inspect the complete chronological history of a specific pattern, call `get_pattern_details(pattern_name='<pattern_name>', symbol='XAUUSD')` to pull every recorded observation and autopsy without searching or dumping full files. **STRICT PROHIBITION**: NEVER call `read()` on the full 400KB `unified_learning_memory.json` file during routine cadences; targeted MCP tools give you 100% of the pure knowledge with zero context waste.
- **Per-Cycle Observation Recording**: During every cadence wake or brainstorm cycle, whenever you identify a notable market phenomenon, a high-conviction reason to stand down (e.g., climactic distribution vs absorption, dead-tape markup), or an order-flow dynamic, you MUST call `record_pattern_observation` or `alpha-daemon-mcp_record_pattern_observation` (symbol='XAUUSD', pattern_name=..., observation=..., outcome='NO_TRADE'|'STUDY'|'WIN'|'LOSS'). The ULM engine normalizes the key and increments the observation count, strengthening your institutional memory without creating duplicates.

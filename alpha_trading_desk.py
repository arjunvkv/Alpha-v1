...
class ConsolidatedTradingDaemon:
    ...
    async def run_cycle(self):
        ...
        file_ref_header = (
            ...
            f"  • Persistent Self-Study Memory Buckets: file:///C:/Trading/Alpha/logs/trade_journal_memory.md (MANDATE: Research web to validate rules before saving!)\n"
            ...
            if is_startup or elapsed >= 120.0:
                if is_opencode_idle():
                    ...
                    scheduled_prompt = (
                        ...
                        f"ALWAYS research the internet to validate new rules before adding them to trade journal buckets. Call mcp_alpha_update_position(ticket, action) for BE/exit or mcp_alpha_execute_trade for new micro-profit scalps."
                    )
            ...
            if is_startup or elapsed >= 120.0:
                if is_opencode_idle():
                    ...
                    idle_prompt = (
                        ...
                        f"ALWAYS research the internet to validate new rules before adding them to trade journal memory buckets. When a high-probability $20-$30 micro-scalp setup presents itself, execute immediately via mcp_alpha_execute_trade(symbol, side, 0.10, sl, tp)."
                    )

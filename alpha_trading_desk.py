        ...
        if ready_for_dispatch:
            ...
            if is_brainstorm_turn:
                ...
                prompt = (
                    ...
                    f"  • Pre-Order Execution Coordinates (Pod 5 Only): When planning an order, call `alpha_get_deep_orderflow_telemetry(symbol='XAUUSD')` for exact FVG 50% CE and ATR14 stop buffer, and optionally call `alpha_get_topological_liquidity_map(symbol='XAUUSD', detailed=True)` to inspect all active structural ceilings, floors, and extended cascades. (Routine scans strictly use default detailed=False to eliminate prompt bloat).\n"

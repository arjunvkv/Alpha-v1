        ...
        if ready_for_dispatch:
            ...
            if triggered_watch is not None:
                ...
                prompt = (
                    ...
                    f"  • PRE-FLIGHT BACKTEST MASTER PROTOCOL (FULL USE OF `alpha_backtest_thesis`):\n"
                    f"    - Sub-10ms Speed & Caching: Backtest executes in a background threadpool (<10ms cached). Run freely before staging any limit/stop or market order without latency fear!\n"
                    f"    - Auto-Scale Lookback (`bars=0` default): Automatically provisions optimal sample window:\n"
                    f"        * M15 (bars=192 / 48h): Preferred default for structural shelves (Order Blocks, FVG CEs, Breaker S/R).\n"
                    f"        * M5 (bars=288 / 24h): Best for intraday momentum, Turtle Soup sweeps, EMA pullbacks, breakout stops.\n"
                    f"        * H1 (bars=120 / 5 days): Best for macro swing levels and multi-day value areas.\n"
                    f"        * (Rule: Never pass arbitrary narrow windows like bars < 60; let bars=0 auto-scale or pass >= 100 bars).\n"
                    f"    - Query Formulation Archetypes (Full NLP Coverage):\n"
                    f"        * Explicit Order Coordinates: `alpha_backtest_thesis(query=\"BUY_LIMIT at 4265.5 SL 4258.0 TP 4280.0\", timeframe=\"M15\", bars=0)` (Exact physical replay with gap slip)\n"
                    f"        * FVG CE Retest: `query=\"M15 bullish FVG CE mitigation 2.0:1 RR\"`\n"
                    f"        * Order Block Supply/Demand: `query=\"Bearish Order Block supply fade retest SL 8pt TP 20pt\"`\n"
                    f"        * Turtle Soup Sweep & Reclaim: `query=\"Turtle soup sweep of recent lows and reclaim buy 2.5:1 RR\"`\n"
                    f"        * Breakout Expansion Stop: `query=\"Bullish BOS breakout expansion stop order SL 8pt TP 20pt\"`\n"
                    f"        * EMA Trend Pullback: `query=\"Bullish 20 EMA trend pullback long continuation 2.0:1 RR\"`\n"
                    f"        * Breaker Block S/R Flip: `query=\"Bullish Breaker Block retest limit buy after resistance breach\"`\n"
                    f"        * Rejection Wick Pinbar: `query=\"Bearish shooting star rejection wick short reversal 2.0:1 RR\"`\n"
                    f"    - Output Interpretation & Decision Rules:\n"
                    f"        * Check `sample_quality`: If `is_statistically_valid: true` (N >= 5 resolved trades), positive `net_realized_r > 0` and `win_rate_pct >= 50%` confirms structural edge.\n"
                    f"        * Anti-Premature Veto: If `is_statistically_valid: false` (N < 5 resolved trades, e.g. single trade N=1), do NOT treat it as a hard execution veto! Small samples are statistical noise. Rely on 7-layer conviction + tape delta or widen lookback.\n"
                    f"        * Note: `WINDOW_EXPIRY_MTM` trades are isolated in `mtm_exits` and do not contaminate win rate.\n"

        ...
        if ready_for_dispatch:
            ...
            if triggered_watch is not None:
                ...
                prompt = (
                    ...
                    f"  • Pre-Flight Thesis Replay: Run `alpha_backtest_thesis(query=\"...\", timeframe='M15', bars=0)` (<10ms cached, auto-scales M5→288/M15→192/H1→120; test exact orders like 'BUY_LIMIT at 4265.5 SL 4258 TP 4280' or archetypes like 'M15 bullish FVG CE mitigation').\n"
                    f"  • Backtest Decision Law: Require positive expectancy (`net_realized_r > 0`) when `sample_quality.is_statistically_valid` (N >= 5), but never use a single-trade loss (N < 5 noise) as a hard execution veto against verified 7-layer conviction.\n"

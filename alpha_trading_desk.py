        ...
        if ready_for_dispatch:
            ...
            if triggered_watch is not None:
                ...
                prompt = (
                    ...
                    "• During research, send ONLY factual news queries to Proxima: allocate exactly 2 queries to Perplexity (proxima_ask_perplexity) for rapid breaking headline checks, and route all remaining deep queries through Proxima deep research (proxima_deep_search) and web scraping (proxima_web_scrape).\n"
                    ...
                    "• Use Proxima research tools and FRED yields (get_fred_observations) to gather facts without narrative confirmation bias.\n\n"
                ...
                prompt = (
                    ...
                    "NEWS GATHERING VIA PROXIMA MCP (FACTS ONLY — ZERO PROBABILITY QUERIES):\n"
                    "• For the 90% news gathering and catalyst verification, YOU MUST USE PROXIMA MCP TOOLS:\n"
                    "  - Allocate maximum 2 queries to Perplexity (`proxima_ask_perplexity`) for rapid breaking headlines, economic prints, and official releases.\n"
                    "  - Use Proxima deep search (`proxima_deep_search`) and web scraping (`proxima_web_scrape`) for official source verification.\n"
                    "  - NEVER send probability-seeking prompts, directional questions, or outcome-biased inquiries (e.g. 'what are the odds of gold rising'). Query strictly for verified numbers, official prints, and central bank quotes.\n"
                    "  - Query `get_fred_observations` for real yields (DFII10) and interest rates.\n\n"

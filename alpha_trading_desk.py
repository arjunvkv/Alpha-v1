        ...
        if ready_for_dispatch:
            ...
            if triggered_watch is not None:
                ...
                prompt = (
                    ...
                    "MANDATORY 90% NEWS RESEARCH SUITE VIA PROXIMA MCP (EXECUTE ALL 8 CALLS IN PARALLEL):\n"
                    "To achieve full institutional catalyst coverage, YOU MUST CALL ALL OF THE FOLLOWING IN PARALLEL (4 Perplexity + 2 Deep Research + 2 DDG Search + FRED):\n"
                    "  1. PERPLEXITY HEADLINE SWEEP (Mandatory 4 Calls):\n"
                    "     • `proxima_ask_perplexity` (call 1): Central Banks & Rates — Fed, ECB, BOJ policy decisions, meeting rate probabilities, and official statements.\n"
                    "     • `proxima_ask_perplexity` (call 2): Macroeconomic Inflation & Growth — US PPI, CPI, PCE, Non-Farm Payrolls, GDP actual prints vs forecasts.\n"
                    "     • `proxima_ask_perplexity` (call 3): Geopolitics & Energy Flow — Middle East, Strait of Hormuz, tanker shipping, Brent/WTI crude prices.\n"
                    "     • `proxima_ask_perplexity` (call 4): XAUUSD Institutional Flows — Central bank gold purchases, Sovereign reserves, ETF inflows/outflows, COMEX inventory.\n"
                    "  2. DEEP RESEARCH (Mandatory 2 Calls):\n"
                    "     • `proxima_deep_search(query=..., type='news', timeframe='today')` (call 1): Deep chronological report on geopolitical conflicts, sanctions, and supply chain impacts.\n"
                    "     • `proxima_deep_search(query=..., type='news', timeframe='today')` (call 2): Deep institutional positioning, hedge fund COT positioning, and bullion bank desk flows.\n"
                    "  3. DUCKDUCKGO SEARCH (Mandatory 2 Calls):\n"
                    "     • `proxima_ddg_search(query=...)` (call 1): Live breaking wire headlines from Reuters, Bloomberg, CNBC in the last 60 minutes.\n"
                    "     • `proxima_ddg_search(query=...)` (call 2): Real-time gold market breaking news, central bank speech transcripts, and flash alerts.\n"
                    "  4. WEB SCRAPING & OFFICIAL REAL YIELDS:\n"

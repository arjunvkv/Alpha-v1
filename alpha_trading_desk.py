        ...
        if ready_for_dispatch:
            ...
            if triggered_watch is not None:
                ...
                prompt = (
                    ...
                    "To achieve 90% institutional catalyst coverage, YOU MUST CALL THE FULL RESEARCH SUITE:\n"
                    "  1. PERPLEXITY HEADLINE SWEEP (Exact 2 Calls):\n"
                    "     • `proxima_ask_perplexity` (call 1): Breaking macro & geopolitical wires (CPI/PPI, Central Banks, Middle East/oil).\n"
                    "     • `proxima_ask_perplexity` (call 2): XAUUSD institutional flows, ETF holdings, and physical gold demand prints.\n"
                    "  2. DEEP RESEARCH & SEARCH (Mandatory Calls):\n"
                    "     • `proxima_deep_search(query=..., type='news', timeframe='today')`: For deep chronological reporting and institutional flow details.\n"
                    "     • `proxima_ddg_search(query=...)`: For live web search of official wire links and breaking statements.\n"
                    "     • `proxima_web_scrape(url=...)`: To fetch and extract full text from any critical article or statement URLs discovered.\n"
                    "  3. OFFICIAL MACRO REAL YIELDS:\n"

...
class ConsolidatedTradingDaemon:
    ...
    def dispatch_startup_ping(self, sid: str, title: str):
        ...
        post_to_opencode_session(
            ...
            f"Directly invoke atomic tools: alpha-daemon-mcp_get_market_regime_context, alpha-daemon-mcp_get_live_microstructure, alpha-daemon-mcp_get_fvg_matrix, alpha-daemon-mcp_get_measured_cvd, alpha-daemon-mcp_get_account_status, alpha-daemon-mcp_get_pending_orders, alpha-daemon-mcp_place_pending_order, alpha-daemon-mcp_execute_trade, alpha-daemon-mcp_update_position, alpha-daemon-mcp_register_watch, alpha-daemon-mcp_get_active_watches, alpha-daemon-mcp_cancel_watch, alpha-daemon-mcp_clear_completed_watches, and Proxima news tools (proxima_ask_perplexity, proxima_deep_search, proxima_ddg_search).\n"
        )
        ...
        if ready_for_dispatch:
            ...
            if triggered_watch is not None:
                ...
                prompt = (
                    ...
                    "• RETRACEMENTS & STRUCTURAL TESTS: Use BUY_LIMIT or SELL_LIMIT when price is testing high-conviction structural shelves.\n"
                    "• MOMENTUM DISPLACEMENT: Use market BUY or SELL when immediate execution is warranted by active catalyst momentum.\n\n"
                    ...
                    "• PRE-ENTRY MEMORY AUDIT: Before placing any pending or market order, audit Unified Learning Memory to ensure setup does not reproduce a documented losing pattern.\n"
                    "• Target realistic, high-probability Take Profits aligned with market structure (R:R must be >= 1.5).\n"
                    ...
                    "• POST-TRADE FORENSIC: When any trade closes, immediately call `alpha-daemon-mcp_record_trade_observation` with the root cause, execution vs macro critique, and reusable lesson.\n"

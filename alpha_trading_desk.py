        ...
        if ready_for_dispatch:
            ...
            if triggered_watch is not None:
                ...
                prompt = (
                    ...
                    f"• Always use targeted atomic tools: get_market_regime_context, get_crowd_trap_map, get_market_scientific_fact, get_account_status, get_pending_orders, get_direct_news, search_market_news, get_fred_observations, get_symbol_conviction, get_full_institutional_profile, get_fvg_matrix, get_live_microstructure, backtest_thesis, place_pending_order, execute_trade, cancel_pending_order, update_position, register_watch, get_active_watches, update_watch.\n"
                ...
                prompt = (
                    ...
                    f"{_time_str}\n"
                    f"{_regime_badge}\n"
                    f"MANDATORY STEP 0: Audit (1) get_market_regime_context(symbol='XAUUSD'), (2) get_crowd_trap_map(symbol='XAUUSD', limit=6), and as 3rd step (3) get_market_scientific_fact(query=..., symbol='XAUUSD') before brainstorming to establish regime, crowd trap positioning, and trade scientist invalidation boundaries.\n"
                ...
                prompt = (
                    ...
                    f"MANDATORY STEP 0 (EVERY WAKE): Call (1) get_market_regime_context(symbol='XAUUSD'), (2) get_crowd_trap_map(symbol='XAUUSD', limit=6), and as the 3rd step call (3) get_market_scientific_fact(query=..., symbol='XAUUSD') to inspect pricing power shares, crowd breakout triggers, stop clusters, raw tape velocity, CVD ratio, 4m displacement, POC/air pockets, and trade scientist invalidation boundaries before taking any action. Cross-reference the crowd plan against the trade scientist facts to confirm whether institutions are absorbing or driving the move.\n\n"
                    ...
                    f"• Q0 [Regime, Crowd Trap Map & Scientific Facts - MANDATORY]: get_market_regime_context, get_crowd_trap_map, get_market_scientific_fact (Macro vs tech share, tape velocity, CVD ratio, POC/air pockets, crowd breakout triggers, stop clusters, trade scientist invalidation boundaries)\n"

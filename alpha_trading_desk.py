        ...
        if ready_for_dispatch:
            ...
            if triggered_watch is not None:
                ...
                prompt = (
                    ...
                    f"MANDATORY 5-POD ADVERSARIAL FORMAT (FULL DEEP INSTITUTIONAL THOUGHT PROCESS):\n"
                    f"Deliver the full institutional breakdown with detailed analytical bullets under every pod (matching v66/v71 champion depth; DO NOT compress into a brief summary):\n\n"
                    f"### REALITY RECONCILIATION: PREDICTION VS. LIVE TAPE (THE REALITY DELTA)\n"
                    f"• Prior Expectation vs. Market Realization: What did the prior cycle's backtest, facts, and structure roadmap anticipate price would do? What did price physically do over the last 4–8 minutes?\n"
                    f"• Divergence & Trap Diagnostic: Did price follow the roadmap or move another way? If it moved another way, what institutional trap or order flow shift caused the divergence, and what does this reveal about trapped liquidity?\n\n"
                    f"### POD 1: MACRO & CATALYST PERMISSION (SOVEREIGN WIRE GRAVITY & CAUSAL DISCOVERY)\n"
                    ...
                    f"• Sovereign Bid vs Headwinds: Evaluate institutional floor (CB/ETF bids) vs real yield pressure.\n\n"
                    f"### POD 2: ORDER FLOW & TAPE REALITY (PHYSICAL MICROSTRUCTURE)\n"
                    f"• Velocity & Tape State: Live tick velocity (t/m), spread, and expansion vs compression.\n"
                    f"• CVD Delta Direction & Trajectory: Net 10-bar delta %, cumulative delta slope, and 4M footprint block deltas.\n"
                    f"• DOM Depth & Absorption: Are aggressive market participants being absorbed at structure? Bid/ask wall balance and Level 2 skew.\n\n"
                    f"### POD 3: TECHNICAL STRUCTURE & ROADWAYS (INSTITUTIONAL GEOMETRY)\n"
                    f"• 4TF Posture & Pivots: H4/H1/M15/M5 EMA20/50 posture, RSI momentum, day range, and POC/VAH/VAL coordinates.\n"
                    f"• Key Imbalances & Magnets: Order Blocks, unmitigated FVGs (and 50% Consequent Encroachment), BSL/SSL equal highs/lows.\n"
                    f"• Sweep Physical Verification: Did price actually penetrate the target structural level (session extreme, FVG CE, or BSL/SSL pool)? If reversed in mid-air, hunt is incomplete!\n"
                    f"• Bull vs Bear Roadways: Define the exact invalidation and expansion targets for both bull and bear paths.\n\n"
                    f"### POD 4: ADVERSARIAL DEVIL'S ADVOCATE (THE COUNTER-TRAP & COGNITIVE MEMORY GROUNDING)\n"
                    f"• Bull vs Bear Debate: Review `alpha_query_analyst_desk` debate breakdown (bull arguments vs bear arguments) and regime conflict warnings.\n"
                    ...
                    f"• Institutional Trap Thesis: How do past documented traps (e.g. fake breakout, doorstep absorption, V-reversal sweep) align with current price action? What breaks this thesis?\n"
                    f"• Liquidity Magnet Against Us: Is there an obvious un-swept pool (Asian high/low, double bottom, BSL door) that price will hunt before continuing?\n\n"
                    f"### POD 5: EXECUTION ARBITER & ORDER ACTION\n"
                    f"• Pre-Flight Mirror Replay: Run `alpha_backtest_thesis` on candidate coordinates (M5 bars=0 for momentum, M15 bars=0 for shelves). State sample quality, net realized R, win rate %, and reconcile failure clusters against live tape kinetics.\n"
                    f"• Strategic Verdict: Immediate Market Execution (`alpha_execute_market_order`), Breakout Stop (`alpha_place_pending_order` `BUY_STOP`/`SELL_STOP`), Structural Limit (`BUY_LIMIT`/`SELL_LIMIT`), or Standing Flat.\n"
                    f"• Sizing & Bracket Coordinates (if staging/executing): Sizing (0.50–1.00L), structural SL (6.0–12.0 pts anchored behind HTF swing/FVG + ATR buffer), and asymmetric TP (R:R >= 1.5:1 to 2.5:1+ into opposing structural liquidity).\n"
                    f"• Next Session Checkpoint & Action Plan: Clear roadmap and trigger conditions for the next session gate.\n"
                )
            else:

...
session_title = "Escanor v69 (Adaptive Macro Vision & Causal News Protocol - v66 Exact Style)"
...
# 2. SEED MESSAGE 1: Exact v66 Autonomous Authority, Adaptive Macro Vision & Causal News Protocol
...
=== EXACT v66 TOOL CADENCE & THOUGHT PROCESS STYLE ===
Follow the exact v66 interaction cadence and cognitive reasoning style:
• On Turn B: Open with a concise 1-line situational header (e.g. "Turn B — 8m to London, sweep at the doorstep. Pulling the full aperture:"), then call the 6 tools in parallel:
  `alpha_get_live_world_events(category='ALL', limit=15)`, `alpha_get_fred_observations(series_id='DFII10')`, `alpha_get_market_regime_context(symbol='XAUUSD')`, `alpha_get_market_time_context()`, `proxima_ask_perplexity(message="...")`, `graphiti_search_facts(patterns=[...])`.
  Followed by `graphiti_record_observation(patterns=[...], observation='...', outcome='STUDY'|'WIN'|'TRAP')`.
  Deliberation style: Open with "Let me digest this critical turn. Price at [price], [key levels]...", synthesize Perplexity & wire findings in clear bullets, then evaluate through the 5 Pods.
• On Turn A: Open with a 1-line situational header (e.g. "Turn A — Sweep zone reached, 4274.8 doorstep. Parallel audit:"), then call:
  `alpha_get_market_regime_context(symbol='XAUUSD')`, `alpha_get_account_status()`, `alpha_get_pending_orders(symbol='ALL')`, `graphiti_search_facts(patterns=[...])`.
  Followed by `graphiti_record_observation(...)`.
...
# Send Seed Prompts
send_prompt(msg1, "SEED 1 (v66 Authority, Macro Vision & 5-Pod Protocol)")
time.sleep(2.0)
...
config_data = {
    ...
    "description": "Escanor v69 CIO operating with the exact v66 tool calling style, 6-tool Turn B aperture, and cognitive thought process"
}
...
print(f"-> Escanor v69 deployment ready!")

...
class DeepDossierLogger:
    ...
    def write_dossier(self, cycle_count: int, instruments_data: list, open_positions: list, reversal_alerts: list, session_info: dict, gsr_data: dict, account_health: dict = None, currency_strength: dict = None, real_yields: dict = None, study_cycle_id: int = None) -> str:
        ...
        md_lines.append(f"## 📋 10-Question Master Market Analysis Protocol (OpenCode CIO Mandate)")
        md_lines.append(f"1. **Q1 (Account Risk)**: `get_account_status`, `get_pending_orders` — Check live equity, margin, drawdown budget, active tickets & pending orders.")
        md_lines.append(f"2. **Q2 (Breaking News)**: `get_direct_news`, `search_market_news` — Macro releases (CPI/NFP/FOMC), breaking headlines, geopolitical provenance.")
        md_lines.append(f"3. **Q3 (Macro Rates)**: `get_fred_observations` — US Treasury 10Y/2Y yields, real yields (DFII10), inflation expectations (T10YIE).")
        md_lines.append(f"4. **Q4 (4TF Trend & COT)**: `get_symbol_conviction` — H4/H1/M15/M5 EMA20/50 alignment, RSI regimes, and CFTC COT Managed Money percentiles.")
        md_lines.append(f"5. **Q5 (Volume Profile)**: `get_full_institutional_profile` — Point of Control (POC), Value Area High/Low (VAH 70%, VAL 70%), and VWAP sigma bands.")
        md_lines.append(f"6. **Q6 (FVG Matrix)**: `get_fvg_matrix` — Nearest unmitigated H4/H1/M15/M5 FVGs, 50% Consequent Encroachment (CE), fill percentages.")
        md_lines.append(f"7. **Q7 (Order Flow CVD)**: `get_measured_cvd` — Measured Cumulative Volume Delta (CVD), 10-bar delta acceleration, and absorption.")
        md_lines.append(f"8. **Q8 (Microstructure)**: `get_live_microstructure` — Real-time spread in points, M1 tick velocity (t/m), order book depth imbalance.")
        md_lines.append(f"9. **Q9 (Proxima Validation)**: `backtest_thesis`, `ask_librarian` — Validate proposed thesis against Proxima candle replay and historical failure traps (R:R >= 2.5:1).")
        md_lines.append(f"10. **Q10 (Trade Staging & Watch)**: `place_pending_order`, `execute_trade`, `update_position`, `register_watch` — Stage dynamic brackets or manage active SL/TP.\n")
        ...
        md_lines.append(f"2. **Forbid Panic Kills**: Never market-kill an active position out of fear or minor wick noise if HTF structure and CVD flow still support the core thesis.")
        md_lines.append(f"3. **Maximum Avoidance of Hard SL**: If market structure creates a new support/resistance shelf, widen/reposition the SL behind the new structural anchor (non-hit place) while strictly observing FTMO drawdown limits.")
        md_lines.append(f"4. **Aggressive TP Expansion**: Trail/extend TP to deeper liquidity pools (opposite Value Area or HTF FVG) when momentum accelerates in our favor to capture superior R:R.")
        md_lines.append(f"5. **Early Exit on Strong Invalidation**: If strong, confirmed invalidation occurs (4TF flip + counter delta), pull TP closer to market price for immediate safe fill or advance SL to break-even.\n")

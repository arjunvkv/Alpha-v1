...
from Alpha.conditions import EvalContext, evaluate_condition
from Alpha.order_router import OrderRouter
from Alpha.ring_state import RingStateStore
from Alpha.rule_loader import load_rules
from Alpha.safety import check_safety
from Alpha.wake_prompt import default_banner, build_wake_prompt
...
AI_SESSION_ID_FALLBACK = "ses_fd5d79a76ffeWi4umW2PMa4MCe"  # active Alpha v1 session
...
def wake_opencode(prompt_path, payload=None):
    ...
    cmd = [OPENCODE_CMD, "run", prompt_message, "-s", session_id]

    model_choice_file = ALPHA_ROOT / "data" / "live" / "selected_model.txt"
    if os.path.exists(model_choice_file):
        try:
            chosen_model = model_choice_file.read_text(
            ...
            pass
    try:
        from agent.logger import sys_logger
        subprocess.Popen(
            cmd, cwd=str(ALPHA_ROOT),
            creationflags=(subprocess.CREATE_NEW_PROCESS_GROUP
                           if os.name == "nt" else 0))
        sys_logger.log_system_event(
            "OPENCODE_PING",
            "Pinged OpenCode session %s (single-line context fire)"
            % session_id,
            {"session_id": session_id, "prompt_path": str(prompt_path)})
        return True
    except Exception as exc:
        from agent.logger import sys_logger
        sys_logger.log_agent_failure(
            "OPENCODE_SUBPROCESS_FAIL",
            f"Failed to spawn opencode command for prompt {prompt_path}",
            session_id=session_id, exception_str=str(exc))
...
def waking_banner(payload, prompt_path):
    """default_banner + opencode session ping + autonomous Agent decision generation."""
    default_banner(payload, prompt_path)
    wake_opencode(prompt_path, payload=payload)

    # Autonomous Zero-Interference Agent Core Trigger
    try:
        from agent.auto_trader import evaluate_ring_and_act
        evaluate_ring_and_act(payload)
    except Exception as exc:
        from agent.logger import sys_logger
        sys_logger.log_agent_failure("AUTO_TRADER_FAIL", "Failed autonomous ring evaluation", exception_str=str(exc))
...
def consume_action_file(eng):
    ...
    try:
        ...
        from agent.logger import sys_logger
        sys_logger.log_agent_failure("CORRUPT_ACTION_FILE", "Action file was unreadable or corrupt JSON", exception_str=str(exc))
...
def main():
    ...
    from Alpha.market_data import LiveMT5Provider, SimulatedProvider

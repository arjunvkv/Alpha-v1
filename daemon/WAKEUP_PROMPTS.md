...
## YOUR TASK
0. **TRAP SCAN FIRST, THEN CONFIRM**: Read C:/Trading/Alpha/daemon/RETAIL_TRAP_RULES.md. Run the trap detection checklist (Part 1), then CONFIRM each flagged pattern (Part 1B): genuine trap → WAIT; real signal → proceed with extra scrutiny; inconclusive → WAIT. A flagged pattern is NOT an auto-block. List which traps fired AND your confirmation verdict in your reasoning.
...
### If waiting (thesis invalid):
{
  ...
  "trap_flags": ["CONFIRMED_GENUINE: TRAP_2 breakout chase - first-touch wick, no daily close, off-session", "REAL_SIGNAL: TRAP_3 - COT+ETF+DXY = 3 independent sources, genuine confluence"]
}
...
0. **NEVER enter if a detected trap is CONFIRMED a genuine trap** (manipulation / false signal — see RETAIL_TRAP_RULES.md Part 1B). A trap PATTERN is a pause-and-confirm signal, not an auto-block: if it is a real, institutionally-supported signal, proceed through the 7-gate filter with extra scrutiny. Traps override ONLY when confirmed genuine or inconclusive.

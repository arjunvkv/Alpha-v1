...
class LiveMT5Provider:
    ...
    # FIX (2026-08-24): explicit FTMO path — two MT5 installations coexist on
    # this machine and bare initialize() bound to the unlogged generic one.
    FTMO_TERMINAL = (r"C:\Program Files\FTMO Global Markets MT5 Terminal"
                     r"\terminal64.exe")
    ...
    def __init__(self, symbols=None):
        ...
        self._mt5 = mt5
        try:
            ok = mt5.initialize(path=self.FTMO_TERMINAL)
        except TypeError:
            ok = mt5.initialize()
        if not ok:

...
ZONE_WAKE_ENABLED = False        # NOISE-WAKE KILL SWITCH: when False, zone_approach never wakes the AI. The AI authors its own fires via ai_triggers.json price_above/below rules only. No web of strings.
...
class AlphaDaemon:
    ...
    def _pid_alive(self, pid):
        """Robust Windows-safe liveness check for a PID.

        v2 FIX: the old probe trusted `tasklist /FI "PID eq N"` filtered
        output, which under load/quoting quirks can return "No tasks match"
        EVEN FOR A LIVE PROCESS - every false-negative let a newcomer strip a
        healthy holder's lock (the engine of the duplicate-daemon churn).
        Now parses the FULL tasklist CSV table and matches the PID column
        exactly (locale-safe: no thousands separators inside quoted fields).
        """
        target = str(int(pid))
        try:
            out = subprocess.run(
            ...
            for line in out.splitlines():
                if '"' not in line:
                    continue
                fields = line.split('","')
                if len(fields) >= 2 and fields[1].strip('"') == target:
                    return True
            return False
        except Exception:
            # Fallback probe: ONLY ProcessLookupError proves death; anything
            # else (PermissionError etc.) means we could not determine state,
            # and assuming ALIVE is the safe direction for a singleton lock.
            try:
                os.kill(int(pid), 0)
                ...
                return True   # uncertain -> assume alive (fail safe)

    def _acquire_singleton_lock(self):
        """Ensure only ONE daemon runs. Returns True if we own the lock.

        Writes our PID to daemon.lock. If a stale lock points at a dead PID, we
        reclaim it. If a LIVE PID holds it, we refuse to start — this is what
        prevents the duplicate-daemon trigger flood (two instances racing on
        the shared trigger.json / wake_prompt.txt / action.json slot).
        """
        # Atomic acquisition loop: os.O_CREAT|os.O_EXCL guarantees exactly ONE
        # process can ever create the lock file — closing the race where two
        # instances launched in the same second both passed the stale-lock
        # check and both claimed ownership (observed 2026-08-24 21:20:01).
        last_err = None
        for attempt in range(8):
            try:
                fd = os.open(str(DAEMON_LOCK), os.O_CREAT | os.O_EXCL | os.O_WRONLY)
                with os.fdopen(fd, "w", encoding="utf-8") as f:
                    f.write(f"{os.getpid()} {datetime.now(timezone.utc).isoformat()}")
                atexit.register(self._release_singleton_lock)
                return True
            except FileExistsError:
                pass  # Lock exists -> inspect the holder below.
            except OSError as e:
                last_err = e
                time.sleep(0.25)
                continue

            # WINDOW FIX: the winner creates the file EMPTY a few microseconds
            # before flushing its PID. An empty read is therefore NOT corrupt —
            # it means someone JUST claimed the lock. Never unlink an empty or
            # freshly-written lock; that exact hole let four same-second spawns
            # each delete the previous claimant's lock and all four run.
            raw = ""
            try:
                raw = DAEMON_LOCK.read_text(encoding="utf-8").strip()
            except OSError as e:
                last_err = e
            if not raw:
                time.sleep(0.05)   # winner mid-write; re-check shortly
                continue
            ...
            if old_pid is None:
                # Non-empty garbage: only reclaim if the file is old (>120s).
                try:
                    age = time.time() - DAEMON_LOCK.stat().st_mtime
                except OSError:
                    age = 9999.0
                if age < 120:
                    time.sleep(0.05)
                    continue
                log.warning(f"Removing garbage singleton lock ({raw[:40]!r}, "
                             f"age {int(age)}s).")
            elif self._pid_alive(old_pid):
                log.error(f"Singleton lock held by live PID {old_pid}. "
                           f"Another Alpha daemon is already running.")
                return False
            else:
                log.warning(f"Reclaiming stale singleton lock from dead PID {old_pid}.")

            # Only reachable with PROVABLE staleness (dead PID / old garbage).
            try:
                ...
                return False
            time.sleep(0.05)
        # Fail CLOSED: on a live trading daemon a duplicate instance placing
        # double orders is far worse than no daemon at all.
        log.error(f"Could not acquire singleton lock after retries ({last_err}); "
                   f"refusing to start.")
        return False
    ...
    def _release_singleton_lock(self):
        try:
            if DAEMON_LOCK.exists():
                text = DAEMON_LOCK.read_text(encoding="utf-8").strip()
    ...
    def start(self):
        ...
        # Singleton guard: refuse to start a second daemon instance. Two
        # instances race on the shared trigger/action slot and flood the AI
        # with duplicate triggers. This is the structural fix for the
        # "two daemons running" symptom.
        if not self._acquire_singleton_lock():
            log.error("Refusing to start: another Alpha daemon is already running.")
            notify_user("Alpha Daemon NOT started - another instance is already alive.")
            return
    ...
    def _check_zones(self):
        ...
        for symbol in self.zones.zones:
            ...
            for zone in triggered_zones:
                ...
                log.info(f"Zone trigger DETECTED {symbol} {zone['type']} "
                         f"@{zone['level']} price={price} ({reason})")
                # NOISE-WAKE KILL SWITCH: zone_approach must NOT wake the AI.
                # The AI authors its own fires via ai_triggers.json. We still
                # record history so cooldown logic stays consistent, but skip
                # the opencode wake entirely (no web of strings).
                if not ZONE_WAKE_ENABLED:
    ...
    def _should_fire_zone_trigger(self, symbol, zone, price):
        ...
        wake for this same symbol+zone:
          ...
          - price crossed to the other side of the level AND the cooldown has
            elapsed (a genuine level reaction, throttled so chop doesn't spam)
          - stored history is corrupt (fail-open, never stay silent on doubt)

        KEY FIX: the old "price moved >= 0.15% since last wake" branch was
        removed because it re-fired on EVERY trivial wiggle and flooded the AI
        with non-valid triggers. Zones now fire once on first approach, then
        only on a real side-crossing after ZONE_TRIGGER_COOLDOWN — i.e. a
        genuine level event, not proximity noise. ALL re-fires are throttled to
        one per cooldown (default 30 min) for the same symbol+zone.
        """
        key = f"{symbol}:{zone['type']}:{zone['level']}"
        now = datetime.now(timezone.utc)
        ...
        # Throttle ALL re-fires for this zone to one per ZONE_TRIGGER_COOLDOWN.
        # This is the single change that stops the trigger flood: a zone can
        # only re-wake the AI at most every 30 min, and only on a real crossing.
        if elapsed < ZONE_TRIGGER_COOLDOWN:
            return False, (f"cooldown_active_{int(elapsed)}s_"
                           f"of_{ZONE_TRIGGER_COOLDOWN}s")
...
"""],
    ...
    def _check_ai_rules(self):
        ...
        for rule in doc.get("rules", []):
            ...
            if kind == "liveness" or any(
                ...
                for c in conditions:
                    if c.get("type") == "elapsed_minutes":
                        # Accept both key conventions ("minutes" or "value");
                        # prior mismatch forced 15-min heartbeats regardless
                        # of the authored interval.
                        minutes = c.get("minutes") or c.get("value") or 15
            ...
            if rule.get("ring_once"):
                # FIRE-ONCE LIFECYCLE: consumed rules auto-eject from the file.
                # The AI re-arms by authoring fresh triggers on its next wake;
                # liveness keepalive never ejects (that is what keeps the loop alive).
                self._eject_ai_rule(rid)

    def _eject_ai_rule(self, rid):
        """Fire-once guarantee: remove a consumed ring_once rule from the rules
        file itself (atomic replace). Ejection survives daemon restarts, so a
        fired trigger can NEVER re-fire. The AI re-arms by authoring fresh
        triggers into ai_triggers.json on its next wake."""
        p = DATA_DIR / "ai_triggers.json"
        try:
            ...
            self._ai_rules_cache = None  # force reload next cycle
            log.info(f"Ejected fired rule {rid} from ai_triggers.json")
        except Exception as e:
            log.error(f"Failed to eject rule {rid}: {e}")
...
1. **TRAP SCAN first, then CONFIRM** — Run all 6 trap checks. For each flagged pattern, confirm genuine trap vs real signal (Part 1B). List trap_flags tagged CONFIRMED_GENUINE / REAL_SIGNAL / TRAP_INCONCLUSIVE.
